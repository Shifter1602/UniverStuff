...
int CCommonView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//формируем список изображений для дерева объектов
	...
	m_reportbutton.Create("Таблица", WS_CHILD|WS_VISIBLE|WS_BORDER, CRect(430, 180, 500, 200), this, IDC_REPORTBUTTON);
...
m_tree.SetImageList(&m_treeimagelist,TVSIL_NORMAL);//привязка списка изображений

//создаем корневой элемент
TVITEM tvitem;  //структура одного элемента дерева
	tvitem.mask = TVIF_TEXT|TVIF_IMAGE|TVIF_SELECTEDIMAGE;
	tvitem.pszText= "Корень";
	tvitem.cchTextMax = 7;
	tvitem.iImage=0;
	tvitem.iSelectedImage=1;
	TVINSERTSTRUCT tvinsert; //структура для вставки новго объекта в дерево
	tvinsert.hParent=TVI_ROOT; //Это корневой элемент,у него нет родителя
	tvinsert.hInsertAfter=TVI_FIRST; //Это первый объект на данном уровне иерархии
	tvinsert.item = tvitem; //Экземпляр структуры,содержащий описание вставляемого объекта
	HTREEITEM hRoot = m_tree.InsertItem(&tvinsert); //вставить корневой объект
	//создаем элемент подчененный корневому
	tvitem.pszText= "Элемент 0";
	tvitem.cchTextMax = 10;
	tvitem.iImage= 1;
	tvitem.iSelectedImage= 2;
	tvinsert.hParent=hRoot; //Это элемент,у которого родитель-корень
	tvinsert.hInsertAfter=TVI_FIRST; //Это первый объект на данном уровне уровне
	tvinsert.item = tvitem; //Экземпляр структуры,содержащий описание вставляемого объекта
	HTREEITEM hItem0 = m_tree.InsertItem(&tvinsert); //вставить новый элемент

	tvitem.pszText= "Элемент 0.0";
	tvitem.cchTextMax = 10;
	tvitem.iImage= 0;
	tvitem.iSelectedImage= 0;
	tvinsert.hParent=hItem0; //Это элемент,у которого родитель-корень
	tvinsert.hInsertAfter=TVI_FIRST; //Это первый объект на данном уровне уровне
	tvinsert.item = tvitem; //Экземпляр структуры,содержащий описание вставляемого объекта
	HTREEITEM hItem1 = m_tree.InsertItem(&tvinsert); //вставить новый элемент

	tvitem.pszText= "Элемент 0.1";
	tvitem.cchTextMax = 10;
	tvitem.iImage= 1;
	tvitem.iSelectedImage= 1;
	tvinsert.hParent=hItem0; //Это элемент,у которого родитель-корень
	tvinsert.hInsertAfter=TVI_LAST; //Это первый объект на данном уровне уровне
	tvinsert.item = tvitem; //Экземпляр структуры,содержащий описание вставляемого объекта
	HTREEITEM hItem2 = m_tree.InsertItem(&tvinsert); //вставить новый элемент

	tvitem.pszText= "Элемент 0.2";
	tvitem.cchTextMax = 10;
	tvitem.iImage= 2;
	tvitem.iSelectedImage= 2;
	tvinsert.hParent=hItem0; //Это элемент,у которого родитель-корень
	tvinsert.hInsertAfter=TVI_LAST; //Это первый объект на данном уровне уровне
	tvinsert.item = tvitem; //Экземпляр структуры,содержащий описание вставляемого объекта
	HTREEITEM hItem3 = m_tree.InsertItem(&tvinsert); //вставить новый элемент

	tvitem.pszText= "Элемент 1";
	tvitem.cchTextMax = 10;
	tvitem.iImage= 1;
	tvitem.iSelectedImage= 2;
	tvinsert.hParent=hRoot; //Это элемент,у которого родитель-корень
	tvinsert.hInsertAfter=TVI_LAST; //Это первый объект на данном уровне уровне
	tvinsert.item = tvitem; //Экземпляр структуры,содержащий описание вставляемого объекта
	HTREEITEM hItem4 = m_tree.InsertItem(&tvinsert); //вставить новый элемент

	tvitem.pszText= "Элемент 1.1";
	tvitem.cchTextMax = 10;
	tvitem.iImage= 0;
	tvitem.iSelectedImage= 0;
	tvinsert.hParent=hItem4; //Это элемент,у которого родитель-корень
	tvinsert.hInsertAfter=TVI_LAST; //Это первый объект на данном уровне уровне
	tvinsert.item = tvitem; //Экземпляр структуры,содержащий описание вставляемого объекта
	HTREEITEM hItem5 = m_tree.InsertItem(&tvinsert); //вставить новый элемент

	tvitem.pszText= "Элемент 1.2";
	tvitem.cchTextMax = 10;
	tvitem.iImage= 1;
	tvitem.iSelectedImage= 1;
	tvinsert.hParent=hItem4; //Это элемент,у которого родитель-корень
	tvinsert.hInsertAfter=TVI_LAST; //Это первый объект на данном уровне уровне
	tvinsert.item = tvitem; //Экземпляр структуры,содержащий описание вставляемого объекта
	HTREEITEM hItem6 = m_tree.InsertItem(&tvinsert); //вставить новый элемент

	tvitem.pszText= "Элемент 1.3";
	tvitem.cchTextMax = 10;
	tvitem.iImage= 2;
	tvitem.iSelectedImage= 2;
	tvinsert.hParent=hItem4; //Это элемент,у которого родитель-корень
	tvinsert.hInsertAfter=TVI_LAST; //Это первый объект на данном уровне уровне
	tvinsert.item = tvitem; //Экземпляр структуры,содержащий описание вставляемого объекта
	HTREEITEM hItem7 = m_tree.InsertItem(&tvinsert); //вставить новый элемент

	// создаем расширенное текстовое поле
	m_richedit.Create(WS_CHILD|WS_VISIBLE|WS_BORDER|ES_AUTOVSCROLL|ES_MULTILINE, CRect(180, 260, 398, 360), this, IDC_RICHEDITVIEW);
	m_bold.Create("Жирный", WS_CHILD|WS_VISIBLE|WS_BORDER, CRect(400, 270, 490, 290), this, IDC_BOLDBUTTON);
	m_left.Create("Влево", WS_CHILD|WS_VISIBLE|WS_BORDER, CRect(400, 290, 490, 310), this, IDC_LEFTBUTTON);
	m_right.Create("Вправо", WS_CHILD|WS_VISIBLE|WS_BORDER, CRect(400, 310, 490, 330), this, IDC_RIGHTBUTTON);
	m_center.Create("По центру", WS_CHILD|WS_VISIBLE|WS_BORDER, CRect(400, 330, 490, 350), this, IDC_CENTERBUTTON);

	
	// формируем элемент "календарь"
...
BOOL CCommonView::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	if (tvdispinfo->hdr.code == TVN_BEGINLABELEDIT) // если пользователь включил режим редактирования подписи
		CEdit* pEdit = m_tree.GetEditControl(); // получаем указатель на редактируемый объект
...
void CCommonView::OnBoldButton()
{
	...
	charformat.cbSize = sizeof(CHARFORMAT); // определяем размер структуры
	charformat.dwMask = CFM_BOLD; // работает со стилем "полужирный"
	m_richedit.GetSelectionCharFormat(charformat); // загружаем в экземпляр структуры CHARFORMAT
	// формат символов выделенного фрагмента текста
	if (charformat.dwEffects & CFM_BOLD) // если в выделенном фрагменте шрифт уже полужирный
		charformat.dwEffects = 0; // отменить полужирный шрифт
	else charformat.dwEffects = CFE_BOLD; // применить полужирный шрифт
	m_richedit.SetSelectionCharFormat(charformat); // назначаем для выделенного фрагмента текста
	// формат символов в соответствии с измененным экземляром структуры CHARFORMAT
	m_richedit.SetFocus(); // вернуть фокус клавиатуры в элемент "Расширенное текстовое поле"
