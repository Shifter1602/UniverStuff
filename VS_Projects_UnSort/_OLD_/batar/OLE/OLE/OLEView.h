...
{
static CLIPFORMAT m_cfObjectDescriptorFormat; // экземпляр структуры формата буфера обмена
...
public:
	...
	COleDropTarget m_droptarget; // объектная переменная для регистрации окна представления в качесиве адресата перетаскивания
	CPoint m_dragpoint; // точка указателя мыши
	CSize m_dragsize; // габариты перетаскиваемого объекта
	CSize m_dragoffset; // отступ указателя мыши от левого верхнего угла объекта
	BOOL m_OKtoDrop; // переменная, запоминающая, можно ли бросать объект в данном месте
	BOOL m_focusrectangledrawn; // переменная, запоминающая, была ли нарисована фокусирующая рамка
