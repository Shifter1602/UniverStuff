	...
	: COleClientItem(pContainer)
{
	...
	m_rect = CRect(10,10,210,210); // инициализация прямоугольника для хранения размеров 
	// внедренного объекта
...
BOOL COLECntrItem::OnChangeItemPosition(const CRect& rectPos)
{
	...
	m_rect = rectPos; // запоминаем новое положение и размер внедренного объекта
	GetDocument()->SetModifiedFlag(); // установить признак изменения документа
	GetDocument()->UpdateAllViews(NULL); // обновить все копии текущего документа
