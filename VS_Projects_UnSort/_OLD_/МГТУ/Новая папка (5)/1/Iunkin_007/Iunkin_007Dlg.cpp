...
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_EDIT1, z);
	DDX_Control(pDX, IDC_EDIT2, p1);
	DDX_Control(pDX, IDC_EDIT3, p2);
	DDX_Control(pDX, IDC_EDIT4, p3);
//	DDX_Control(pDX, IDC_EDIT5, p4);
	DDX_Control(pDX, IDC_CHECK1, ChBox1);
}
...
{
	char x[200];
	z.GetWindowText ( x, 200 ); 
	sscanf ( x , "%i", &Size );
	p1.GetWindowText ( x, 200 ); 
	sscanf ( x , "%i", &pr1 );
	p2.GetWindowText ( x, 200 ); 
	sscanf ( x , "%i", &pr2 );
	p3.GetWindowText ( x, 200 ); 
	sscanf ( x , "%i", &pr3 );
	if(ChBox1.GetState()==1)Mask=1;else Mask=3;
	...
	SetThreadPriority(H1,pr1);
	SetThreadAffinityMask(H1,Mask);
	CloseHandle(H1);
	HANDLE H2= CreateThread(0,0,StartSortIns,B,0,0);
	SetThreadPriority(H2,pr2);
	SetThreadAffinityMask(H2,Mask);
	CloseHandle(H2);
	HANDLE H3= CreateThread(0,0,StartSortQck,C,0,0);
	SetThreadPriority(H3,pr3);
	SetThreadAffinityMask(H3,Mask);
	CloseHandle(H3);
}
